---
title: Ubuntu20.04(GLIBC < 2.34)网络代理软件推荐
date: 2026-05-09 16:38:59
tags:
---
- Q：目前Ubuntu很多代理软件，为什么推荐Mihomo？
- A：原因有两点：
    - 第一点：本人现在使用的梯子是支持Hysteria2 / Vless Reality，很多客户端不支持这个协议，而且本人之前使用的代理软件V2rayn和NekoRay都会漏掉许多节点。
    - 第二点：本人使用的ubuntu版本较老（GLIBC < 2.34），现在很多支持Hysteria2 / Vless Reality协议的代理软件，如Hiddify，Fclash等都没办法安装。
- 亲测使用Mihomo最方便，而且机场节点一个不漏
- 以下是整理的 **Ubuntu20.04(GLIBC < 2.34)网络代理软件推荐笔记**，适合作为开发者文档长期保存。
---

# 🚀 Ubuntu 下 Mihomo (Clash Meta) 配置笔记

## 1. 环境背景

* **系统版本**: Ubuntu (GLIBC < 2.34)
* **核心选择**: Mihomo (原 Clash Meta) - 兼容性强，支持 Hysteria2/Vless Reality。
* **冲突处理**: 避开了 MinIO 占用的 `9090` 端口。

## 2. 核心程序准备

为了规避系统依赖库（GLIBC）版本过低的问题，采用 **gz 压缩包形式** 的静态编译版，不使用 `.deb` 安装包。

```bash
# 1. 解压下载的 gz 包
gunzip mihomo-linux-amd64-v1.19.24.gz

# 2. 重命名为简洁的名称
mv mihomo-linux-amd64-v1.19.24 mihomo

# 3. 赋予可执行权限
chmod +x mihomo

```

## 3. 获取配置文件 (YAML)

利用 `curl` 模拟 Clash 客户端请求头，确保从机场订阅链接获取的是正确的 YAML 格式，并开启 Meta 协议支持。

```bash
# 替换为你的订阅链接，注意末尾的 ?flag=meta
curl -L -A "clash-verge/1.0" -o config.yaml "https://feed1.chitanda-eru.com/oosaka/1dc0a8c305b1887877c0d4ea992ac0c2?flag=meta"

```

## 4. 关键配置修改

通过 `nano config.yaml` 修改以下核心参数，解决端口冲突并启用控制台。

* **API 控制端口**: 将 `external-controller` 从 `9090` 改为 `9091`（避开 MinIO）。
* **混合代理端口**: 确认 `mixed-port` 为 `7890`（用于系统代理连接）。
* **TUN 模式 (可选)**: 若需全局接管，可配置 `tun: {enable: true, ...}` 并使用 `sudo` 启动。

## 5. 启动与控制

### 后端启动

```bash
./mihomo -f config.yaml

```

### 前端控制台

访问网页版控制面板进行节点切换和测速：

* **推荐面板**: [http://clash.razord.top/](https://www.google.com/search?q=http://clash.razord.top/) (HTTP 版，无拦截风险)
* **后端地址**: `http://127.0.0.1:9091`
* **密钥**: 默认为空

## 6. 系统流量接管

在 **Ubuntu 系统设置 -> 网络 -> 网络代理** 中配置：

* **方法**: 手动 (Manual)
* **HTTP/HTTPS/Socks**: `127.0.0.1`
* **端口**: `7890`

---

为了方便你以后快速查阅和执行常用指令，我为你准备了一个交互式的 **Mihomo 命令工具箱**。

### 进阶建议

作为研究员，如果你经常需要使用 `git clone` 或 `pip install`：

* **临时代理**: 在终端执行 `export https_proxy=http://127.0.0.1:7890`。
* **永久代理**: 可将上述命令写入 `~/.bashrc` 文件末尾。

这套配置现在已经能够完美支持你项目所需的 **Hysteria2** 和 **Vless Reality** 节点了。